    ...
    handleInput(eventName: string, event: KeyboardEvent) {
        if (eventName == "keydown") {
            switch (event.code) {
                case 'ArrowUp':
                case 'Numpad8':
                    this.player.mixins['Movable'].execute({dx: 0, dy: -1}, this.current_site);
                    break;
                case 'ArrowDown':
                case 'Numpad2':
                    this.player.mixins['Movable'].execute({dx: 0, dy: 1}, this.current_site);
                ...
                case 'ArrowLeft':
                case 'Numpad4':
                    this.player.mixins['Movable'].execute({dx: -1, dy: 0}, this.current_site);
                    break;
                case 'ArrowRight':
                case 'Numpad6':
                    this.player.mixins['Movable'].execute({dx: 1, dy: 0}, this.current_site);
                    break;
                case 'Numpad7':
                    this.player.mixins['Movable'].execute({dx: -1, dy: -1}, this.current_site);
                    break;
                case 'Numpad9':
                    this.player.mixins['Movable'].execute({dx: 1, dy: -1}, this.current_site);
                    break;
